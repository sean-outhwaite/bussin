...
const [page, setPage] = useState('home')
  ...
  return (
    <div className='h-100 w-100 min-h-screen bg-white dark:bg-gray-800 px-6 py-8 ring shadow-xl ring-gray-900/5' >
      ...
      <nav className='bg-[#ebf5fb] px-4 py-3 rounded-lg mx-auto flex flex-row font-extrabold mt-0 mb-2 space-x-8 rtl:space-x-reverse text-lg text-[#2d7caf]'> <Link onClick={()=> setPage('home')} className={`hover:underline ${page === 'home' ? 'text-[#cb0076] underline decoration-2' : 'text-[#2d7caf]'}`} to='/'>Map</Link> <Link className={`hover:underline ${page === 'board' ? 'text-[#cb0076] underline decoration-2' : 'text-[#2d7caf]'}`} to='/board' onClick={()=> setPage('board')}>Board</Link></nav>
